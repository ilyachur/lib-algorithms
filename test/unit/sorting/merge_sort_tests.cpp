...
class merge_sort_tests: public ::testing::Test {
protected:
...
TEST_F(merge_sort_tests, sort_array_with_default_compare) {
    int A [6] = {10, -5, 2, 40, -30, 3};

    Sorting::Merge::sort(A, 6);
    ASSERT_THAT(A, testing::ElementsAre(-30, -5, 2, 3, 10, 40));
}

TEST_F(merge_sort_tests, sort_array_with_custom_compare) {
    int A [6] = {10, -5, 2, 40, -30, 3};
    Sorting::Merge::sort(A, 6, std::greater<int>());

    ASSERT_THAT(A, testing::ElementsAre(40, 10, 3, 2, -5, -30));
    Sorting::Merge::sort(A, 6, [](int a, int b) {
        return a < b;
    });
...
TEST_F(merge_sort_tests, sort_array_with_compare_strings) {
...
TEST_F(merge_sort_tests, sort_big_random_array) {
    ...
    Sorting::Merge::sort(unsortedArr, 10000);
    for (unsigned int i = 0; i < 10000; i++) {
